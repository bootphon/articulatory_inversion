...
def traitement_general_usc_timit(speaker,N):
  ...
  #  N=len(EMA_files)
    ...
    def treat_ema_files(i):
        ...
        np.save(os.path.join(path_files_treated,"ema_filtered",EMA_files_2[i]),ema)
    N_2 = len(EMA_files_2)
    ...
    def for_normalisation():
        ...
        for i in range(len(EMA_files_2)):
            ema_i = np.load(os.path.join(path_files_treated,"ema_filtered",EMA_files_2[i]+".npy"))
            ...
            np.save(os.path.join(path_files_treated, "ema_filtered", EMA_files_2[i]), ema_i)
        ...
        np.save("moving_average_ema_" + speaker, smoothed_moving_average)
    ...
    def normalization_and_lowpass(std_ema, mean_ema, smoothed_moving_average, mean_mfcc,std_mfcc):
        ...
        for i in range(len(EMA_files_2)):
            ...
            ema = np.load(os.path.join(path_files_treated,"ema_filtered",EMA_files_2[i]+".npy"))
            mfcc = np.load(os.path.join(path_files_treated,"mfcc",EMA_files_2[i]+".npy"))

            if len(ema)!= len(mfcc):
                print("pbmmm 2",EMA_files_2[i],len(ema),len(mfcc))
            ema = (ema - smoothed_moving_average[i,:] + mean_ema) / max(std_ema)
...
traitement_general_usc_timit(speakers[1],N)
traitement_general_usc_timit(speakers[2],N)
traitement_general_usc_timit(speakers[3],N)
