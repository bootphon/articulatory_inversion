...
def traitement_general_haskins(N_max):
    ...
    def traitement_haskins(speaker,N_max=N_max):
        ...
        def read_ema_and_wav(k):
            ...
            wav_data = data[0][2][:,0]
            librosa.output.write_wav(os.path.join(root_path,"Donnees_brutes",corpus,speaker,
                                                  "wav",EMA_files[k]+".wav"),wav_data, sampling_rate_wav)
            wav,sr = librosa.load(os.path.join(root_path,"Donnees_brutes",corpus,speaker,"wav",EMA_files[k]+".wav")
                                  ,sr=sampling_rate_wav_wanted)
