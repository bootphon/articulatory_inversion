...
import scipy
import torch
from Training.velum_modele import learn_velum
...
def add_vocal_tract_old(speaker,max="All"):
 #   print("adding vocal tracts for speaker {}".format(speaker))
    ...
    def add_velum(mfcc):
        ...
        model_to_load = os.path.join(root_folder, "Training", "saved_models", "modeles_valides",
                                     "modele_velum.txt")
    ...
    if speaker in ["msak0", "fsew0","maps0","faet0","mjjn0","ffes0","falh0"]:
        ...
        wav_path = os.path.join(root_folder, "Raw_data","mocha", speaker)
        sampling_rate_wav = 16000

    elif speaker in ["F1", "F5", "M1","M3"]:
        speaker_2 = "usc_timit_" + speaker
        wav_path = os.path.join(root_folder, "Raw_data", "usc_timit",speaker,"wav_cut")
        sampling_rate_wav = 20000

    elif speaker == "MNGU0":
        speaker_2 = speaker
        wav_path = os.path.join(root_folder, "Raw_data", speaker,"wav")
        sampling_rate_wav= 16000
    elif speaker in ["F01", "F02", "F03", "F04", "M01", "M02", "M03", "M04"]:
        speaker_2 = "Haskins_"+speaker
        wav_path = os.path.join(root_folder, "Raw_data", "Haskins_IEEE_Rate_Comparison_DB",speaker,"wav")
        sampling_rate_wav= 44100

    mfcc_path = os.path.join(root_folder, "Preprocessed_data", speaker,"mfcc")
    files_path = os.path.join(root_folder, "Preprocessed_data", speaker,"ema_filtered_norma")

    if not os.path.exists(os.path.join(root_folder,"Preprocessed_data",speaker,"ema_VT")):
        os.makedirs(os.path.join(root_folder,"Preprocessed_data",speaker,"ema_VT"))

    files = glob.glob(os.path.join(root_folder,"Preprocessed_data",speaker,"ema_VT","*"))
    for f in files:
        os.remove(f)
   ...
   #     ema[:, 16:18] = velum_xy
        np.save(os.path.join(root_folder,"Preprocessed_data",speaker,"ema_VT",EMA_files_names[i]),ema)
