...
arti_fsew0 = np.load(os.path.join(fileset_path,"Y_fsew0.npy"),allow_pickle=True)
arti_msak0 = np.load(os.path.join(fileset_path,"Y_msak0.npy"),allow_pickle=True)
...
class test_filter(torch.nn.Module):
    ...
    def filter_layer(self,y):
        ...
        for i in range(self.output_dim):
            ...
            difference = int(abs(traj_arti_smoothed.shape[2] - L) / 2)
...
def train_model(lr=0.05,n_epochs=20):
    ...
    for epoch in range(n_epochs):
        ...
        y_pred_smoothed= model_velum(x) #,y_pred_smoothed
...
train_model(n_epochs=5,lr=0.01)
